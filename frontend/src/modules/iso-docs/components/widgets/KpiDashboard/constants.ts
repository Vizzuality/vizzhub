...
export const ISO_CYCLE_START_MONTH = 3;

export function getIsoCycle(year: number): IsoCycle {
  return {
    year,
    startYear: year,
    startMonth: ISO_CYCLE_START_MONTH,
    endYear: year + 1,
    endMonth: 2,
  };
}
...
export function getCycleMonths(cycle: IsoCycle): MonthColumn[] {
  ...
  for (let i = 0; i < 12; i++) {
    const month = ((ISO_CYCLE_START_MONTH - 1 + i) % 12) + 1;
    const year = month >= ISO_CYCLE_START_MONTH ? cycle.startYear : cycle.endYear;
    months.push({
      year,
      month,
...
export const INDICATOR_DEFINITIONS: Record<string, IndicatorDef> = {
  ...
  cost_variance_pct: {
    name: 'Cost Variance',
    description:
      'EVM Cost Variance over budget. Signed — negative means overrun relative to the value delivered, positive means under-spent. Replaces the legacy clamped budget_variance.',
    formula:
      '(EV - AC) / BAC = percent_completed - cost_to_date / budget_total. Scored on a piecewise-linear normalizer: >= 0 → 100, <= -target → 0.',
  },
  defect_density: {
    name: 'Defect Density',
...
export const DIMENSION_DEFINITIONS: DimensionDef[] = [
  ...
  },
  {
    ...
    description:
      'Software quality across defects, governance, reviews, and failure rates. Capped at 60 if Sev1 incident.',
    formula:
      'weighted_avg(defect_density, escaped_rate, mttr, story_review, governance, pr_review, change_failure_rate, post_contract_tasks). Sev1 cap applied.',
    indicators: [
      'defect_density',
      'escaped_rate',
      'mttr_hours',
  ...
  },
  {
    ...
    formula:
      'weighted_avg(lead_time, commitment_reliability, pr_size, review_turnaround, deployment_frequency)',
    indicators: [
      'lead_time_days',
      'commitment_reliability',
      'pr_size_median',
...
export const MANUAL_KPI_FIELDS: ManualKpiFieldDef[] = [
  { key: 'name', label: 'KPI Name', type: 'string', required: true },
