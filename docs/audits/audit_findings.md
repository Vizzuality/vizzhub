...
- **[partially fixed 2026-05-15] Global metrics aggregation includes archived/cancelled projects + equal-weighted** — `backend/app/modules/scorecard/services/global_metrics_service.py:75` (`calculate_and_store`), :134 (`_average_indicators`), :163 (`_average_scores`). Resolution: aggregator now JOINs `ProjectDB` and filters to `ACTIVE_PORTFOLIO_STATUSES = (LIVE, FINISHED)` — `PROPOSAL` projects no longer pollute the portfolio average. `strategic_impact.strip().lower()` so trailing whitespace doesn't fall to None. Sub-issue #3 (the #14 normalizer cluster) is already fixed. **Sub-issue #2 (equal weighting) — TO DISCUSS 2026-05-15:** user wants to revisit the weighting policy in a separate conversation (weight by budget? by headcount? leave equal?). **Sub-issue #4 (stale-snapshot leakage) — UI WORK 2026-05-15:** instead of a backend fix, the decision is to surface staleness in the UI. See companion fix on `/scorecard` index: stale projects (LIVE with last capture > 35 days old) get a warning icon next to their card title.
  - Module: `scorecard`
