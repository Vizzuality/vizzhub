...
- **Tracker (11 SUSPICIOUS):** in progress. **5 closed 2026-05-16** (#28, #31, #25, #27, #30). Remaining: #18, #20, #21, #24, #26 (structural / currency family). **Pre-deploy gate:** check prod `SELECT id, date, base_rate FROM reporting_periods WHERE base_rate <= 0` — if any rows exist, migration `071_period_base_rate_gt0` will fail.
