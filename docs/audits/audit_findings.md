...
- ~~**Unused error state in `InvoiceDetail`**~~ **[fixed d8c62300]** — `frontend/src/modules/tracker/pages/InvoiceDetail.tsx` is now a thin route wrapper; error state lives in the new `InvoiceDetailContent` where it's actually rendered via `setError` → in-card banner.
  - Module: `frontend / tracker / pages`
...
- **Tracker (11 SUSPICIOUS):** **all 11 closed 2026-05-16** (#18, #20, #21, #24, #25, #26, #27, #28, #30, #31 — plus #29 was already OK). Cross-currency thread-through deferred — `exchange_rate_service` supports `as_of` but tracker cost paths not yet wired (separate roadmap item). **Pre-deploy gate verified clean 2026-05-16**: prod `SELECT id, date, base_rate FROM reporting_periods WHERE base_rate <= 0` → `COUNT: 0` (via SSM + asyncpg on `hub-backend`). Migrations `071_period_base_rate_gt0` and `072_cv_pct_replaces_bv` safe to deploy. **Pushed to `dev` + `main` 2026-05-16**; Deploy workflow had to be triggered manually (`workflow_dispatch`) because the push HEAD was a `[skip ci]` docs commit, which suppresses the entire push's CI/CD run. **Post-deploy:** invalidate score cache + recalc scorecard history (Cost dim semantics changed via #18). New finding #40 logged (forecastFinal vs chart cap mismatch) — to revisit.
- **Capacity (4 SUSPICIOUS):** **all 4 closed 2026-05-16** (`24971b18`). Partial-reporter and over-reporter sub-issues deferred (don't happen in production — reporting is binary). Internal/admin work now exposed as `other_pct` across all Insights views.
- **Frontend:** **fully closed 2026-05-16** (#38 `5319e868`, #39 + #40 `ae0c3da4`). Zero open items in the calculations audit.
...
- **EVM Cost Variance (CV = EV − AC) not implemented; only CPI + clamped overrun** — `backend/app/modules/scorecard/services/normalizers/indicators.py:85` (EV), `:79` (CPI), and `backend/app/modules/scorecard/services/normalizers/base.py:125-148` (budget_variance overrun). **[FIXED 2026-05-16 — commit `f92ff36b` + post-deploy patch `a4593681` (Cost Variance KPI tile added to Budget & Schedule strip on project detail page — backend rename alone was invisible to the user; only the ISO Docs KPI widget surfaced the label). 5-column responsive grid, signed € / signed %, red/green/neutral tones.]**
...
- **EAC (Estimate at Completion) uses a non-standard time-based forecast, ignoring CPI/BAC/EV** — `frontend/src/modules/tracker/components/BurnDashboard.tsx:82` (`forecastFinal`), with helpers `weightedMonthlyAvg` and `buildForecastPoints` in the same file. **[FIXED 2026-05-16 — commit `371b031b` + post-deploy patches `a4593681` (Y-domain clamp at 3× budget + amber color + endpoint label) and `f9950e12` (Recharts `<Line>` → `<Area fill="none">` because `<Line>` is silently dropped inside `<AreaChart>`). Visual QA surfaced both: EVM line was off-screen on high-CPI projects and didn't render at all due to the Recharts gotcha. Tests now sit alongside but assert only the legend; SVG-path assertion still missing — logged as test-gap, not blocker.]**
  - Module: `tracker`
...
- **ETC (Estimate to Complete) not exposed separately; only implicit inside EAC** — sister finding to row #20. `frontend/src/modules/tracker/components/BurnDashboard.tsx:82`. **[FIXED 2026-05-16 — commit `371b031b`. Subsumed by the new EVM forecast line; remaining-cost is now visible as the gap between EAC_CPI and AC on the chart. Refund/gap inflation in the time-trend forecast still open — out of scope for this fix.]**
  - Module: `tracker`
...
- **Cost-to-date aggregation — base_rate=0 raises 500; checkpoint premise about hours×rate is wrong** — `backend/app/modules/tracker/services/cost_service.py:25-114` (per-part calc), aggregated by `backend/app/modules/tracker/services/aggregation_service.py:42-140` via `SUM(report_part.cost)`. **[base_rate=0 FIXED 2026-05-16 — commit `c4aaeac9`. Currency-mismatch sub-issue remains open, tracked under #24/#26.]**
  ...
  - Other edge cases handled: percentage=None → cost=None; rate_id=None / dedication=None → cost=None (excluded from aggregate, not zeroed); deleted Rate row → cost=None; estimated/percentage≤0 parts → excluded; missing project settings → DEFAULT_RATE fallback; DB CHECK `percentage∈[0,1]` + `cost >= 0`.
...
- **ECB currency conversion — no rate=0 guard, no historical lookup, no staleness check** — `backend/app/core/services/exchange_rate_service.py` (`convert_to_eur` :97, `get_latest_rate` :82, `currency_to_code` :30, `get_available_currencies` :112). **[rate=0, None/empty code, historical `as_of` FIXED 2026-05-16 — commit `417aaa4f`. Stale-rate alerting + DB `CHECK rate > 0` + Decimal quantize deferred. Unblocks #24/#25 cross-currency thread.]**
  - Module: `core/services`
...
- **Mood aggregation — estimated reports contaminate the average + trend silently skips current month + no `/trend` test** — `backend/app/modules/tracker/api/moods.py:38-217`. **[FIXED 2026-05-16 — commit `f63345a8`. Estimated filter added on monthly + trend queries; current-month exclusion pinned by test; first /trend coverage shipped. Anti-banker-rounding + named_feedback pagination still deferred.]**
  - Module: `tracker`
...
- **Capacity FA averages — partial reporters drag the average down; over-reporters not clamped; internal/admin invisible** — `backend/app/core/services/capacity_insights.py:388-417` (`_aggregate_fa_period`), user query at `:316-318`, per-user totals at `:326-349`, on-leave skip at `:403`. **[FIXED 2026-05-16 — commit `24971b18`. Internal/other work now exposed as `other_pct` field across all three Insights views (overview, FA detail, user detail), mirroring the existing pattern in `get_allocation_users`. Partial-reporter and over-reporter sub-issues deferred — confirmed not to happen in production (reporting is binary: report 100% or don't report).]**
...
- **Capacity user detail — same project shows twice when split across FAs + no gap segment for partial reporters + sum=1.0 invariant only enforced at Confirm** — `backend/app/core/services/capacity_insights.py:225-271` (user detail), `:116-148` + `:161-168` (FA detail). Confirm gate at `backend/app/modules/tracker/api/reports.py:180-194` (`math.isclose(total, 1.0, rel_tol=1e-4)`). Report-part unique constraint at `backend/app/modules/tracker/models/report_part.py:21` = `(project_id, report_id, functional_area_id)` — NOT unique on `(project_id, report_id)`. **[FIXED 2026-05-16 — commit `24971b18`. `GROUP BY (period_id, project_id) + SUM(percentage)` dedupes multi-FA splits; internal projects now roll into the `__other__` pseudo-row alongside billable projects in the same response shape allocation uses. Confirm-edit drift sub-issue deferred (separate concern, no production incident reported).]**
  - Module: `capacity`
  - Issues:
...
- **Reportable-users filter inconsistent — 3 paths leak inactive/exempt users** — `backend/app/core/services/capacity_insights.py:627` (`get_allocation_projects`), `backend/app/modules/capacity/api/planner.py:179` (planner main query), `capacity_insights.py:197` (`get_capacity_user_detail`). **[FIXED 2026-05-16 — commit `24971b18`. Centralized `_reportable_user_filter()` helper now applied at 5 call sites: get_allocation_projects, get_allocation_users, get_capacity_fa_detail, get_capacity_insights, get_reportable_users. Planner leak corrected: real leak was `_get_overallocation_warnings` at `:179`, NOT the main query (which already had both filters). User-detail now 404s for inactive/exempt users.]**
  - Module: `capacity`
...
- **On-leave detection: "total=0" semantic too narrow; full-PTO reporters drag FA averages** — `backend/app/core/services/capacity_insights.py:159` (FA detail), `:403` (FA aggregator), `:602-604` (allocation_users). **[FIXED 2026-05-16 — commit `24971b18`. New `_is_on_leave(total, absence)` helper: `total ≤ 0` OR `math.isclose(absence, 1.0)` OR `math.isclose(absence, total)`. Applied in `_aggregate_fa_period` and `get_capacity_fa_detail`. Users with 100% absence no longer drag the FA billable denominator.]**
  - Module: `capacity`
...
- **TypeScript types lie about Decimal-as-string wire format** — Events types (`frontend/src/modules/events/types/events.ts:40-41,123`), Rate type, untyped Stats response. **[FIXED 2026-05-16 — commit `5319e868`. Types now match the wire (`string` for Decimal fields); 2 tsc errors surfaced, both fixed with `Number()` coercion at the boundary. No latent bugs found — existing call sites were already defensive. 1 test mock updated. Tracker/scorecard untouched (already honest, backend coerces to float).]**
...
- **Capacity charts default to oldest 6 months instead of latest** — `frontend/src/modules/capacity/components/ChartPagination.tsx:11-21`, consumers `InsightsChart.tsx:40`, `FADetailChart.tsx:99`, `UserDetailChart.tsx:89`. **[FIXED 2026-05-16 — commit `ae0c3da4`. New `latestChartPage(dataLength)` + `chartPageCount(dataLength)` helpers; lazy initializer `useState(() => latestChartPage(chartData.length))` in all 3 consumers (snaps on mount, user click sticks afterwards). UserDetailChart also resets on userId change. +12 regression tests.]**
  - Module: `capacity`
...
- **BurnDashboard `forecastFinal` KPI tile doesn't match the chart endpoint on long-tail projects** — `frontend/src/modules/tracker/components/BurnDashboard.tsx` (`useChartData` + `forecastFinal`). **[FIXED 2026-05-16 — commit `ae0c3da4`. Option B picked: 24-month cap removed from `buildForecastPoints` in `BurnDashboard.tsx:88-110`. Both forecast lines (time-trend + EVM) chain off `forecast.points.length`, so consistency is automatic. Sane bounds (end_date in past / invalid) preserved. +4 regression tests.]**
  - Module: `tracker`
  - The chart's forecast line caps the projected timeline at `Math.min(remainingMonths, 24)` months, but the `forecastFinal` KPI tile uses the **uncapped** `remainingMonths`. For projects with more than 24 months remaining, the tile shows a number that the chart never reaches visually — UI inconsistency.
  - Surfaced 2026-05-16 by the agent fixing #20/#21. Out of scope for that fix (which was purely additive — new EVM forecast line).
  - Fix: either cap `forecastFinal` to the same 24-month window, or drop the chart cap entirely and let it render whatever the formula produces. Whichever you pick, both surfaces must agree.
  - Added: 2026-05-16 (post-fix #20/#21 — calc-audit row #40)

- **Final-score weights are not validated at runtime; misconfig fails silently** — `backend/app/modules/scorecard/services/calculators/final_score.py:50-127` (consumer) + `backend/app/config.py:274-350` (`ScoringConfig.validate_weights` — informational only, never raises). **[FIXED 2026-05-16 — commit `4b96fc20`.** New `validate_weights_with_totals()` reports `(passed, actual_sum)` per group; `load_scoring_config_from_db` emits a `scoring_config_invalid_weights` structlog warning for every broken group at startup, with the actual sum so ops can see *why* it failed. Legacy `validate_weights()` kept for `/config` endpoint backward compat. Calculator-side renormalisation already prevents nonsense output; this closes the silent-failure half. 4 new unit tests in `tests/test_scoring_weights_validation.py`. **Operational note:** a misconfig surfaces as a Loki warning, not a hard fail — the system stays up (intentional, so an ongoing config edit can't take down the API).**]
  - Module: `scorecard`
