...
async def _get_users_pending_confirmation(
    ...
    result = await db.execute(
        select(UserDB).where(
            ...
            ~UserDB.id.in_(confirmed_user_ids),
        )
    )
...
async def send_report_confirmation_reminder(ctx: dict) -> dict[str, Any]:
    ...
    try:
        ...
        period = await _get_active_period(db)
        if not period:
            return await complete_with_error(
        ...
        if not users:
            ...
            job_run.projects_checked = 0
            job_run.alerts_sent = 0
            await db.commit()
            return {
                "status": "completed",
