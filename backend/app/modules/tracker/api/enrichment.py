...
def _build_report_response(
    report: ReportDB, user: UserDB | None,
) -> ReportResponse:
...
def _build_part_response(
    part: ReportPartDB, project: ProjectDB | None,
) -> ReportPartResponse:
...
async def enrich_reports_batch(
    reports: list[ReportDB], db: AsyncSession,
) -> list[ReportResponse]:
    ...
    return [
        _build_report_response(r, user_map.get(r.user_id))
        for r in reports
    ]
...
async def enrich_parts_batch(
    parts: list[ReportPartDB], db: AsyncSession,
    ...
    project_map: dict[UUID, ProjectDB] = {
        p.id: p for p in result.scalars().all()
    }
    return [
        _build_part_response(p, project_map.get(p.project_id))
        for p in parts
    ]
