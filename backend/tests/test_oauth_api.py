...
class TestOAuthJiraAuthorize:
    ...
    @pytest.mark.asyncio
    async def test_oauth_jira_authorize_generates_state_token(self, client: AsyncClient) -> None:
    ...
    @pytest.mark.asyncio
    async def test_oauth_jira_authorize_redirects_with_state(self, client: AsyncClient) -> None:
...
class TestOAuthJiraCallback:
    ...
    @pytest.mark.asyncio
    async def test_oauth_jira_callback_state_mismatch_rejected(self, client: AsyncClient) -> None:
        """State parameter doesn't match session should be rejected."""
        response = await client.get("/api/oauth/jira/callback?code=test-code&state=wrong-state")
    ...
    @pytest.mark.asyncio
    async def test_oauth_jira_callback_state_missing_rejected(self, client: AsyncClient) -> None:
    ...
    @pytest.mark.asyncio
    async def test_oauth_jira_callback_invalid_code_fails(self, client: AsyncClient) -> None:
    ...
    async def test_oauth_jira_callback_stores_token_in_database(
        ...
        with patch("app.core.api.oauth.OAuthService.exchange_jira_code_for_token") as mock_exchange:
            mock_token = OAuthTokenDB(
                provider="jira",
...
class TestOAuthJiraStatus:
    ...
    @pytest.mark.asyncio
    async def test_oauth_jira_status_requires_authentication(self, client: AsyncClient) -> None:
...
class TestOAuthJiraRefresh:
    ...
    @pytest.mark.asyncio
    async def test_oauth_jira_refresh_requires_authentication(self, client: AsyncClient) -> None:
        """Endpoint requires valid JWT in production mode."""
        response = await client.post("/api/oauth/jira/refresh")
...
class TestJiraDisconnect:
    ...
    @pytest.mark.asyncio
    async def test_disconnect_jira(self, client: AsyncClient, db_session: AsyncSession) -> None:
