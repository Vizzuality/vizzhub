...
class TestCollectVulnerabilities:
    @pytest.mark.asyncio
    async def test_returns_zero_when_no_alerts(self, mock_client) -> None:
        ...
        result = await collect_vulnerabilities(mock_client, "owner/repo")

        assert result["high_severity_vulns"] == 0
        assert result["high_severity_vulns_total"] == 0

    @pytest.mark.asyncio
    async def test_counts_alerts_older_than_30_days(self, mock_client) -> None:
    ...
    @pytest.mark.asyncio
    async def test_returns_zero_when_dependabot_not_enabled(self, mock_client) -> None:
    ...
    @pytest.mark.asyncio
    async def test_returns_zero_when_repo_not_found(self, mock_client) -> None:
    ...
    @pytest.mark.asyncio
    async def test_handles_boundary_29_days(self, mock_client) -> None:
    ...
    @pytest.mark.asyncio
    async def test_handles_31_days(self, mock_client) -> None:
...
class TestGetDependabotAlerts:
    ...
    async def test_returns_none_on_api_error(self, mock_client) -> None:
        ...
        result = await _get_dependabot_alerts(mock_client, "owner", "repo")

        assert result is None

    @pytest.mark.asyncio
    async def test_handles_empty_response(self, mock_client) -> None:
    ...
    @pytest.mark.asyncio
    async def test_filters_by_severity(self, mock_client) -> None:
        """API call should filter by high,critical severity."""
        mock_http = AsyncMock()
        mock_http.get.return_value = MagicMock(status_code=200, json=lambda: [])
        mock_client.get_client = AsyncMock(return_value=mock_http)

        await _get_dependabot_alerts(mock_client, "owner", "repo")
