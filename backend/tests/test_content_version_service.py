...
from app.modules.playbook.models.node import (
    PlaybookNodeDB,  # registers playbook_nodes in Base.metadata
)
...
async def test_get_latest(db_session, version_service):
    node_id = await _create_node(db_session)
    await version_service.save_version(db_session, entity_id=node_id, content="old", user_id=None)
    await version_service.save_version(db_session, entity_id=node_id, content="new", user_id=None)
...
async def test_get_specific_version(db_session, version_service):
    node_id = await _create_node(db_session)
    await version_service.save_version(db_session, entity_id=node_id, content="v1", user_id=None)
    await version_service.save_version(db_session, entity_id=node_id, content="v2", user_id=None)
...
async def test_get_version_returns_none_for_missing(db_session, version_service):
    result = await version_service.get_version(db_session, entity_id=uuid4(), version=99)
    assert result is None
